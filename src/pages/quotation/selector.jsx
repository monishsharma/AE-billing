...
export const getColumns = ({ handleDownload, value, makeQuotationCopy }) => {
  ...
  return [
    {
      field: "Bill",
      renderHeader: () => <strong>Quotation</strong>,
      sortable: false,
      renderCell: (params) => {
        return (
          <Button
    ...
    },
    {
      field: "quotationNo",
      width: 150,
      renderHeader: () => <strong>Quotation No</strong>,
      sortable: true,
    ...
    },
    {
      ...
      renderCell: (params) => {
        const text =
          params?.row?.goodsDescription?.items?.[0]?.description || "-";
        const getTitle = () => {
          const allText = params?.row?.goodsDescription?.items
            ?.map(
              (item) =>
                ` ${item.description}, ₹ ${parseFloat(item.rate || 0).toFixed(2)}`,
            )
            .join("\n");
          return allText || "-";
        };
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <Button
            className="hoverBtn outlinedCustomBtn"
            size="small"
            onClick={(e) => {
              e.stopPropagation();
              makeQuotationCopy(e, params.row);
            }}
          >
            Copy Quotation
          </Button>
