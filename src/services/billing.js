...
export default (api) => {
    ...
    const getInvoiceList = ({company, id, page, month, year, limit}) => {
        const endpoint = id ? `/billing/invoice/${id}` : `/billing/invoice/list/${company}?page=${page}&month=${month}&year=${year}&limit=${limit}`;
    ...
    const generateCSV = ({company, month, year, forGST = false, forUnpaid = false, unpaidInvoicesList}) => {
        return api.post(`/billing/generate-csv?company=${company}&month=${month}&year=${year}&GST=${forGST}&forUnpaid=${forUnpaid}`, unpaidInvoicesList, {
            responseType: 'blob',
        })
    };
